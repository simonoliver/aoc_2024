...
fn add_region_neighbours_at_location(garden_data:&Vec<Vec<char>>, region_location:(i32,i32),found_locations:&mut Vec<(i32,i32)>,processed_locations_list:&mut Vec<bool>) {
    for direction in NEIGHBOUR_DIRECTIONS {
        ...
        if test_location.0>=0 && test_location.0<(garden_data[0].len() as i32) && test_location.1>=0 && test_location.1<(garden_data.len() as i32) { // Bounds check
            ...
            // If not processed and same char
           if !processed_locations_list[location_index as usize] && garden_data[test_location.1 as usize][test_location.0 as usize]==garden_data[region_location.1 as usize][region_location.0 as usize] {
               found_locations.push(test_location); // Add to found locations
               processed_locations_list[location_index as usize]=true; // Mark as processed
...
fn find_regions(garden_data:&Vec<Vec<char>>) -> Vec<Vec<(i32,i32)>> {
    let mut region_list:Vec<Vec<(i32,i32)>>=Vec::new();
    ...
    for (rowindex,row) in garden_data.iter().enumerate() {
        for (column_index,_) in row.iter().enumerate() {
            ...
            if !processed_locations_list[process_index] {
                // New region
                let mut new_region_list:Vec<(i32,i32)>=Vec::new();
